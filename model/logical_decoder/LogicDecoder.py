...
# )
from model.logical_decoder.multi_head_attention import CachedMultiHeadAttention
from model.logical_decoder.mixture_of_operators import MoOLayer
...
@keras_nlp_export("keras_nlp.layers.LogicDecoder")
class LogicDecoder(keras.layers.Layer):
    """Transformer decoder.
    ...
    decoder = keras_nlp.layers.LogicDecoder(
    ...
    def call(
            ...
            use_casual_cross_mask=True,
            training=None,
    ):
        """Forward pass of the LogicDecoder.
        ...
        Args:
            ...
            use_casual_cross_mask: bool, defaults to `True`. If true, a causal
                mask (masking out future input) is applied on the encoder cross-attention sequence
        ...
        if not has_cross_attention and has_encoder_sequence:
            raise ValueError(
                ...
                "`keras_nlp.layers.LogicDecoder` should not change. "
        ...
        if has_cross_attention and (
            ...
            raise ValueError(
                "When calling `keras_nlp.layers.LogicDecoder` with "
        ...
        # attention_output, attention_scores_self = self._self_attention_layer(
        #     query=x,
        #     value=x,
        ...
        # x = x + residual
        # x = self._self_attention_layer_norm(x)
        x, gates_self = self._moolayer_selfattn(
            [residual, x, attention_scores_self],
            training=training,
        )
        ...
        if has_cross_attention:
            ...
            # )
            cross_attention_mask = self._compute_cross_attention_mask(
                encoder_sequence=encoder_sequence,
                encoder_padding_mask=encoder_padding_mask,
                encoder_attention_mask=encoder_attention_mask,
                use_causal_mask=use_casual_cross_mask,
                cross_attention_cache=cross_attention_cache,
                cross_attention_cache_update_index=cross_attention_cache_update_index,
            )

            # print('\n\n\nCross Attention Mask:', cross_attention_mask)

            # # Cross attention block.
            # residual = x
            # if self.normalize_first:
            #     x = self._cross_attention_layer_norm(x)
            # attention_output, attention_scores_cross = self._cross_attention_layer(
            #     query=x,
            ...
            # x = x + residual
            # x = self._cross_attention_layer_norm(x)
            x, gates_cross = self._moolayer_crossattn(
                [residual, x, attention_scores_cross],
                training=training,
            )
    ...
    def _compute_cross_attention_mask(
        ...
        if use_causal_mask:
            batch_size = ops.shape(encoder_sequence)[0]
            input_length = output_length = ops.shape(encoder_sequence)[1]
            # We need to handle a rectangular causal mask when doing cached
            # decoding. For generative inference, `encoder_sequence` will
            # generally be length 1, and `cache` will be the full generation length.
            if cross_attention_cache is not None:
                input_length = ops.shape(cross_attention_cache)[2]
            ...
            causal_mask = compute_causal_mask(
                ...
                (
                    0
                    if cross_attention_cache_update_index is None
                    else cross_attention_cache_update_index
                ),
            )
            return (
                ops.minimum(encoder_mask, causal_mask)
    ...
    def compute_output_shape(self, decoder_sequence_shape, encoder_sequence_shape):
...
if __name__ == '__main__':
    ...
    import tensorflow as tf
    decoder_seq = tf.random.uniform((2, 10, 64))
    encoder_seq = tf.random.uniform((2, 10, 64))
    ...
    print(output.shape)  # Should print (2, 10, 64)
