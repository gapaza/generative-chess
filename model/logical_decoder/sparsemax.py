...
if __name__ == '__main__':
    ...
    def run_case(logits, axis, expected, case_name):
        logits_tf = tf.constant(logits, dtype=tf.float32)
        out = sparsemax(logits_tf, axis=axis).numpy()
        print(f"\n=== {case_name} ===")
        print("Logits:\n", np.array(logits))
        print("Sparsemax:\n", out)
        print("Expected:\n", np.array(expected))
        assert almost_equal(out, expected), f"{case_name} failed!"


    # 1) Simple 2×3 example (from the paper)
    run_case(
        logits=[[-1.0, 0.0, 1.0],
                [-5.0, 1.0, 2.0]],
        axis=-1,
    ...
    # 2) Uniform logits -> uniform output
    run_case(
        logits=[[2.0, 2.0, 2.0]],
        axis=-1,
        expected=[[1 / 3, 1 / 3, 1 / 3]],
        case_name="Uniform logits"
    )

    # 3) Negative values only -> still picks top elements
    run_case(
        logits=[[-3.0, -1.0, -2.0]],
        axis=-1,
        expected=[[0., 1., 0.]],
        case_name="All negatives"
    )
