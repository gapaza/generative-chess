...
critic_loss_coef_warmup = 0.5
critic_loss_coef_final = 0.01
critic_warm_up_epochs = 40
...
class ActorWrapper:
    ...
    def calc_reward(self, board, uci_move, prev_score, engine):
        white_turn = (board.turn == chess.WHITE)

        # Check if illegal move
        if uci_move in config.non_move_tokens:
            self.illegal_moves += 1
            return True, illegal_move_reward, prev_score
        ...
        if board.is_checkmate():
            self.checkmates += 1
            return True, checkmate_reward, prev_score
        if board.is_stalemate() or board.is_insufficient_material() or board.is_seventyfive_moves() or board.is_fivefold_repetition():
            self.draws += 1
            return True, draw_reward, prev_score
        ...
        # reward += move_bonus
        return False, reward, next_score

    def get_actions(self, games, white_turn, games_done, scores, boards):
        ...
        for idx in range(len(games)):
            ...
            game_done, reward, next_score = self.calc_reward(board, action_token, prev_score, self.engine)
    ...
    def train_ppo(self, games, white_turn):
        ...
        for i in range(self.task.train_actor_itr):
            ...
            kl, entr, loss, policy_loss, value_loss = self.train_actor_critic(
                model_moves, cross_moves, model_turns,
                action_buffer, logprobability_buffer, action_mask_tensor, advantage_buffer, logprobs_full, probs_full,
                returns, mask_tensor, critic_coef
            )
        ...
        model_color = 'White' if white_turn is True else 'Black'
        model_color += ' (' + str(self.model_num) + '):'
        # print(model_color, 'return', np.mean(self.flat_rewards), 'kl', kl, 'entr', entr, 'loss', loss, 'policy_loss', policy_loss, 'checkmates', self.checkmates, 'illegal_moves', self.illegal_moves, 'policy_update_itr', policy_update_itr)
        print(model_color, 'avg reward', np.mean(self.flat_rewards), 'value loss', value_loss, 'kl', kl, 'entr', entr, 'loss', loss, 'policy_loss', policy_loss, 'checkmates', self.checkmates, 'illegal_moves', self.illegal_moves, 'policy_update_itr', policy_update_itr)

        self.model_info['checkmates'].append(self.checkmates)
        self.model_info['loss'].append(loss)
        self.model_info['entropy'].append(entr)
        self.model_info['kl'].append(kl)
        self.model_info['return'].append(np.mean(self.flat_rewards))
